...
{
    // requires C++20
    setFace(face, rotl(getFace(face), 16));
}

void Rubiks::rotateFaceCCW(EFACE face)
{
    // requires C++20
    setFace(face, rotr(getFace(face), 16));
}

void Rubiks::rotateFace180(EFACE face)
{
    // requires C++20,
    setFace(face, rotl(getFace(face), 32));
}
...
bool Rubiks::isValidCube() const
{
    // Initialize an array to count the occurrences of each color
    std::array<int, 6> colorCount = {0};

    // Iterate through the cube's facelets and count color occurrences
    for (const auto& facelet : m_cube)
    {
        colorCount[static_cast<int>(facelet)]++;
    }

    // Check if all colors appear exactly 8 times
    for (const int count : colorCount)
    {
        if (count != 8)
        {
            return false;
    ...
    // If all colors appear 8 times, the cube is valid
    return true;
}

void Rubiks::toggleColor(uint8_t index)
{
    if (index >= m_cube.size())
    {
        std::cerr << "Invalid index for toggling color." << std::endl;
        return; // Handle the out-of-range index
    }

    // Get the current color at the specified index
    ECOLOUR currentColor = m_cube[index];

    // Calculate the next color by incrementing the current color (wrapping around)
    ECOLOUR nextColor = static_cast<ECOLOUR>((static_cast<int>(currentColor) + 1) % 6);

    // Update the color at the specified index
    m_cube[index] = nextColor;
}
